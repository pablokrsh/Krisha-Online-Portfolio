...
const tooltipStyle = {
  contentStyle: {
    ...
    fontFamily: 'Inter',
    fontSize: '12px',
    boxShadow: '0 8px 24px rgba(16,47,21,.12)',
  },
}
...
function StatCard({ label, value, suffix = '', isInView, reduced }) {
  ...
  return (
    <div className="card-base card-hover text-center p-6">
      <p className="font-heading text-4xl font-bold text-forest-900">
    ...
    <section id="skills" className="section-padding bg-mint-100" aria-labelledby="skills-heading">
      <div className="container-main" ref={sectionRef}>
        <div className="text-center mb-16" style={anim('fadeUp', 0)}>
          ...
          <h2 id="skills-heading" className="font-heading text-4xl md:text-5xl font-semibold text-forest-900">
            Skills & Analytics
          </h2>
        </div>
                ...
                <BarChart data={weeklyProgress} barSize={32}>
                  ...
                  <Bar dataKey="hours" radius={[8, 8, 0, 0]}>
                    {weeklyProgress.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={index === 4 ? '#EAF1B1' : '#102F15'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
