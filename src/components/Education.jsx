  ...
  return (
    ...
    >
      ...
      <div className="relative flex-shrink-0 w-6 z-10">
        ...
        <div className="relative w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 hover:scale-125" style={{ background: 'var(--el-accent)', color: 'var(--el-card)' }}>
          {item.icon}
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="card-neo card-neo-hover p-6">
          ...
          {item.college && (
            <p className="font-body text-xs flex items-center gap-1 mb-2" style={{ color: 'var(--el-text-muted)' }}>
              <FiMapPin size={10} /> {item.college}
            </p>
          )}
          <p className="font-body text-xs flex items-center gap-1 mb-4" style={{ color: 'var(--el-text-muted)' }}>
          ...
          <div className="flex flex-wrap gap-2">
            {item.highlights.map((h) => (
              <span key={h} className="px-3 py-1 text-xs font-body rounded-full transition-colors duration-300" style={{ background: 'var(--el-track)', color: 'var(--el-text)' }}>
                {h}
              </span>
            ))}
          </div>
    ...
    <section id="education" className="section-padding" style={{ background: 'var(--el-bg-alt)' }} aria-labelledby="education-heading">
      <div className="container-main" ref={sectionRef}>
        ...
        >
          ...
          <h2 id="education-heading" className="font-heading text-4xl md:text-5xl font-semibold" style={{ color: 'var(--el-text)' }}>
            Education
          </h2>
        </div>
