    ...
    <section id="portfolio" className="section-padding" style={{ background: 'var(--el-bg-alt)' }} aria-labelledby="portfolio-heading">
      <div className="container-main" ref={sectionRef}>
        <div className="text-center mb-16" style={anim(0)}>
          ...
          <h2 id="portfolio-heading" className="font-heading text-4xl md:text-5xl font-semibold" style={{ color: 'var(--el-text)' }}>
            Portfolio
          </h2>
        </div>
          ...
          {filtered.map((project, index) => (
            ...
            >
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, color-mix(in srgb, ${project.color} 15%, transparent), color-mix(in srgb, ${project.color} 30%, transparent))`,
                }}
              >
                <FiFolder size={48} style={{ color: 'var(--el-text)', opacity: 0.15 }} className="transition-transform duration-500 group-hover:scale-110" />
                <div className="absolute inset-0 rounded-[18px] flex items-center justify-center transition-all duration-400" style={{ background: 'transparent' }}>
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 scale-75 group-hover:scale-100">
                    <FiExternalLink size={24} style={{ color: 'var(--el-text)' }} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 text-xs font-body rounded-full transition-colors duration-300" style={{ background: 'var(--el-bg)', color: 'var(--el-text)' }}>
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <div className="text-center mt-12" style={anim(0.7)}>
          <p className="font-body text-sm" style={{ color: 'var(--el-text-sub)' }}>More projects coming soon — stay tuned!</p>
        </div>
      </div>
    </section>
  )
